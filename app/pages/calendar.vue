...
const initialValues = computed(() => ({ startAt: calendarTaskStartAt(selectedDay.value), dueAt: null }))
async function load() {
  ;[tasks.value, tags.value] = await Promise.all([
    useDataService().listTasks({ limit: 100 }),
    useDataService().listTags(),
  ])
}
onMounted(load)
const days = computed(() =>
...
<template>
  <div class="contents">
    ...
    ></UDashboardPanel>
    <UModal v-model:open="open" title="タスクを作成" :ui="{ content: 'sm:max-w-3xl' }">
      <template #body>
        <TaskForm :tasks="tasks" :tags="tags" :initial-values="initialValues" @saved="saved" @cancel="open = false" />
      </template>
    </UModal>
  </div>
</template>
